      ...
      >
        <template #header>
          <div class="header-config">
            <span>角色列表</span>
            <el-button type="primary" v-permissions="['power:role:add']" @click="showModal(true)">创建角色</el-button>
          </div>
        </template>
        <template #status="{ row }">
          <el-switch
        ...
        <template #operator="{ row }">
          <el-button type="success" v-permissions="['power:role:edit']" link @click="showModal(true, row)"
            >编辑</el-button
          >
          <el-button type="danger" v-permissions="['power:role:delete']" link @click="handleDeleteRole(row)"
            >删除</el-button
          >
        </template>
      </fs-table>
